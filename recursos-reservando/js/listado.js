...
//Dado un id, busca el objeto del listado que tiene ese id SE REFACTORIZA
Listado.prototype.buscarRestaurante = function(id) {
    var restaurantSelection = this.restaurantes.find(restaurant => restaurant.id === id);
    return restaurantSelection ? restaurantSelection : "No se ha encontrado ningún restaurant";
}

//Se crea funcion que elimine los elementos repetidos de un Array
...
Listado.prototype.obtenerHorarios = function() {
    ...
    //Array donde se van a agregar los rubros
    var horario = this.restaurantes.map(function(restaurant) {
        return restaurant.horarios;
    });
    // Devolvemos los elementos ordenados y sin repetir.
        return eliminarElementosRepetidos(horario).sort();
